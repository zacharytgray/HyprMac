...
- **Menu bar workspace indicator**: Dynamic `MenuBarExtra` label showing workspace state via shared `MenuBarState` observable. Dot-based format where position = workspace number (truncated at last used): `●` active, `◆` active+floating, `○` occupied, `◇` occupied+floating, `·` empty. Diamond shapes indicate workspaces containing floating windows. Dropdown shows rounded badge grid per screen with accent fill for active, subtle background for occupied, and `◇` markers on workspaces with floaters. Updated by `WindowManager.updateMenuBarState()` after every position cache refresh. Toggleable via `config.showMenuBarIndicator`.
